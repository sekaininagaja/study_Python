...
ほとんどの退屈な作業は、コンピュータ上のファイルに限定されません。   
プログラムでWebページをダウンロードできるようになれば、プログラムはインターネットにも広がります。   
リクエストモジュールは簡単にダウンロードを行い、HTMLのコンセプトとセレクタに関するいくつかの基本的な知識を持って、あなたがダウンロードしたページを解析するためにBeautifulSoupモジュールを利用することができます。

しかし、Webベースのタスクを完全に自動化するには、セレンモジュールを使用してWebブラウザを直接制御する必要があります。   
セレンモジュールでは、ウェブサイトにログインしてフォームを自動的に記入することができます。   
Webブラウザはインターネットを介して情報を送受信する最も一般的な方法なので、これはプログラマーツールキットに入れるうえで優れた機能です。
...
- webbrowser
  - ブラウザで指定したURLをひらくだけの機能
- requests
  - 指定したURLからファイルやWebページをダウンロードする
- BeautifulSoup
  - HTMLの解析をおこなう
- selenium modules
  - ブラウザを人間ちっくに扱う。マウスクリックやキーボードボタン押下、フォームの入力も可能。
...
## コマンドラインで使うメーラーを使う
コマンドラインで電子メールアドレスと文字列を受け取り、Seleniumを使用して電子メールアカウントにログインし、文字列の電子メールを指定されたアドレスに送信するプログラムを作成します。   
（このプログラムのために別の電子メールアカウントを設定することもできます）。

これは、あなたのプログラムに通知機能を追加するうまい方法です。   
同様のプログラムを作成して、FacebookまたはTwitterアカウントからメッセージを送信することもできます。

## イメージサイトダウンローダ
FlickrやImgurのようなフォトシェアリングサイトに行き、写真のカテゴリを検索して、結果として得られるすべてのイメージをダウンロードするプログラムを書く。   
あなたは、検索機能を持つ写真サイトで動作するプログラムを書くことができます。

## 2048
2048はタイルを矢印キーで上下左右にスライドさせて組み合わせるシンプルなゲームです。   
上、右、下、左のパターンを何度も繰り返すことで、実際にはかなり高い得点を得ることができます。   https://gabrielecirulli.github.io/2048/ でゲームを開くプログラムを書いて、自動的にゲームをプレイするために、送信、右、下、左のキーストロークを続けてください。

## リンク検証
ウェブページのURLを指定すると、ページ上のすべてのリンクされたページをダウンロードしようとするプログラムを作成します。   
プログラムは、404 "Not Found"ステータスコードを持つページにフラグを立てて、壊れたリンクとして出力しなければなりません。
